...
```
	protected void autowireByType(
		...
		//和autowireByName实现一致，遍历beanWapper的所有PropertyDescriptor，判断PropertyDescriptor可写，既为set方法，PropertyValues包含该名称，且不是简单参数类型（既Enum，Date，Number等或者数组）
		String[] propertyNames = unsatisfiedNonSimpleProperties(mbd, bw);
		for (String propertyName : propertyNames) {
			try {
				//获取属性类型，如果是Object类型，则忽略
				...
				if (Object.class != pd.getPropertyType()) {
					...
					//使用desc的入参类型作为依赖注入的类型，然后根据desc的类型查找所有类型相同或者父类相同的beanName，然后将所有满足条件的beanName添加到autowiredBeanNames里
...
```
根据注入类型解析参数的逻辑处理完成，后边主要逻辑就是将pvs的所有实例填充到beanWapper对应的bean实例，也就是```applyPropertyValues()```:
```
